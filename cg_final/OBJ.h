...
{
public:
    OBJ(string s, double x, double y, double z, double x1,double y1,double z1)
    {
		visible = true;
    ...
    void render(double x, double y, double z, double sx, double sy, double sz) //在坐标 xyz 处 自转 degree角度
    {
		...
		/*Vector v = set_rotation(degree / 3.141592654 * 180, top, Vector(pivot_x, pivot_y, pivot_z));
		Vector tmp = Vector(0, 0, 1) * Vector(sx, sy, sz);
		tmp = Vector(sx, sy, sz) * tmp;
		top = tmp;
		double d = acos(v.dot(tmp) / sqrt(v.dot(v) + tmp.dot(tmp)));
		glRotatef(d / 3.141592654 * 180, sx, sy, sz);*/
		//cout << "rotate:" << d << endl;
		glRotatef(degree / 3.141592654 * 180, pivot_x, pivot_y, pivot_z);
		//cout << degree << endl;
...

        {
           //cout<<"face "<<i<<":"<<MArr[i]<<endl;
            ...
            for (int j = 0; 1 ; j++)
            {
				...
				/*if (fnArr[i][j] != -1) glNormal3f(vnArr[fnArr[i][j] - 1][1], vnArr[fnArr[i][j] - 1][2], vnArr[fnArr[i][j] - 1][0]);

				if (fvArr[i][j] != -1) glVertex3f(vArr[fvArr[i][j] - 1][1], vArr[fvArr[i][j] - 1][2], vArr[fvArr[i][j] - 1][0]);*/
...
//            glVertex3f(0, 0,0);
	...
	Vector set_rotation(float ang, Vector dir1 ,Vector dir)
	{
		dir.normalize();
		...
		Vector result;
		Vector m0(m[0][0], m[0][1], m[0][2]);
		Vector m1(m[1][0], m[1][1], m[1][2]);
		Vector m2(m[2][0], m[2][1], m[2][2]);
		result.x = m0.dot(dir1) + m[0][3];
		result.y = m1.dot(dir1) + m[1][3];
		result.z = m2.dot(dir1) + m[2][3];
		return result;
	}
	Vector get_center(){
...
private:
	...
	Vector top;
	double max_x = 0, max_y = 0, max_z = 0, min_x = 9999, min_y = 9999, min_z = 9999;
	double center_x, center_y, center_z, r, h;//
    int v_num = 0; //记录点的数量
...
//        memset(fnArr,-1,sizeof(fnArr));
		...
		Vector m = Vector(max_x, max_y, max_z);
		Vector c = Vector(center_x, center_y, center_z);
		r = sqrt((m-c).dot((m-c)));
